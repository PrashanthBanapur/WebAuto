...
public class WaitEvent {
	...
	private final int MAX_WAIT = Integer.parseInt(Config.waitMaxTimeout.getValue());
	...
	public synchronized void visible(WebElement element){
		log.info(LoggerUtil.webFormat()+"(WAIT VISIBLE)	-> Element = '{}' ",element);
	...
	public synchronized void notVisible(WebElement element){
		log.info(LoggerUtil.webFormat()+"(WAIT NOT VISIBLE)	-> Element = '{}' ",element);
	...
	public  synchronized void present(By locator){
		log.info(LoggerUtil.webFormat()+"(WAIT  PRESENT)	-> Element = '{}' ",locator);
		wait(ExpectedConditions.presenceOfElementLocated(locator));	
	}
	
	/**
	 *  An expectation with the logical opposite condition of the given condition. <br/>
	 *  Condition , Element is present on the DOM of a page. This does not necessarily mean that the element is visible
	 * @param locator
	 */
	public synchronized void notPresent(By locator){
		log.info(LoggerUtil.webFormat()+"(WAIT NOT PRESENT)	-> Element = '{}' ",locator);
		wait(ExpectedConditions.not(ExpectedConditions.presenceOfElementLocated(locator)));
	}
	...
	public synchronized void clickable(WebElement element){
		log.info(LoggerUtil.webFormat()+"(WAIT  CLICKABLE)	-> Element = '{}' ",element);
	...
	public synchronized void notClickable(WebElement element){
		log.info(LoggerUtil.webFormat()+"(WAIT NOT CLICKABLE)	-> Element = '{}' ",element);
		wait(ExpectedConditions.not(ExpectedConditions.elementToBeClickable(element)));
	}
